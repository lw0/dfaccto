...
{{#x_idwidth}}
  awid     : unsigned ({{x_idwidth}}-1 downto 0);
{{/x_idwidth}}
  awaddr   : unsigned ({{x_addrwidth}}-1 downto 0);
{{^x_noburst}}
  awlen    : t_AxiLen;
  awsize   : t_AxiSize;
  awburst  : t_AxiBurst;
...
{{/x_noattrib}}
  awvalid  : std_logic;
  wdata    : unsigned ({{x_datawidth}}-1 downto 0);
  wstrb    : unsigned ({{x_datawidth}}/8-1 downto 0);
  wlast    : std_logic;
  wvalid   : std_logic;
  bready   : std_logic;
{{#x_idwidth}}
  arid     : unsigned ({{x_idwidth}}-1 downto 0);
{{/x_idwidth}}
  araddr   : unsigned ({{x_addrwidth}}-1 downto 0);
{{^x_noburst}}
  arlen    : t_AxiLen;
  arsize   : t_AxiSize;
  arburst  : t_AxiBurst;
...
type t_{{name}}_sm is record
  ...
  wready   : std_logic;
{{#x_idwidth}}
  bid      : unsigned ({{x_idwidth}}-1 downto 0);
{{/x_idwidth}}
  bresp    : t_AxiResp;
  bvalid   : std_logic;
  arready  : std_logic;
{{#x_idwidth}}
  rid      : unsigned ({{x_idwidth}}-1 downto 0);
{{/x_idwidth}}
  rdata    : unsigned ({{x_datawidth}}-1 downto 0);
  rresp    : t_AxiResp;
  rlast    : std_logic;
  rvalid   : std_logic;
end record;
