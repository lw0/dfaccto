...
with Pkg('dfaccto_axi',
         x_templates={'generic/package.vhd': 'pkg/dfaccto_axi.vhd'}):

  UnsignedType('AxiLen', width=8)

  UnsignedType('AxiSize', width=3)

  UnsignedType('AxiBurst', width=2,
...
def AxiType(name, data_bytes, addr_bits, id_bits=None, add_split=False,
           has_burst=True, has_attr=False, has_lock=None, has_cache=None, has_prot=None, has_qos=None, has_region=None,
           aruser_bits=None, awuser_bits=None, ruser_bits=None, wuser_bits=None, buser_bits=None):
  assert data_bytes in (1, 2, 4, 8, 16, 32, 64, 128), 'Axi only supports power-of-two byte counts up to 128'
  word_idx_bits = uwidth(data_bytes - 1)
  word_addr_bits = addr_bits - word_idx_bits
  assert word_addr_bits >= 0, 'Axi address range must at least include a single data word'
  ...
  if has_burst:
    ...
    tburst = None
    tlast = None
  if (has_cache if has_lock is None else has_lock):
    tlock = T('AxiLock', 'dfaccto_axi')
  else:
    tlock = None
  if (has_cache if has_cache is None else has_cache):
    tcache = T('AxiCache', 'dfaccto_axi')
  else:
    tcache = None
  if (has_cache if has_prot is None else has_prot):
    tprot = T('AxiProt', 'dfaccto_axi')
  else:
    tprot = None
  if (has_cache if has_qos is None else has_qos):
    tqos = T('AxiQos', 'dfaccto_axi')
  else:
    tqos = None
  if (has_cache if has_region is None else has_region):
  ...
  if add_split:
    ...
    variants.append(('{}Wr'.format(name), True, False))
  for variant, has_rd, has_wr in variants:
    TypeC(variant, x_is_axi=True,
          x_has_rd=has_rd, x_has_wr=has_wr,
          x_definition='{{>types/axi.part}}',
          x_format_ms='{{>formats/axi_ms.part}}',
          x_format_sm='{{>formats/axi_sm.part}}',
          x_tlogic=tlogic, x_tresp=tresp,
          x_tdata=tdata, x_tstrb=tstrb, x_taddr=taddr,
          x_twidx=twidx, x_twaddr=twaddr, x_clen=clen, x_csize=csize, x_cburst=cburst,
          x_tid=tid, x_taruser=taruser, x_tawuser=tawuser,
          x_truser=truser, x_twuser=twuser, x_tbuser=tbuser,
          x_tlen=tlen, x_tsize=tsize, x_tburst=tburst, x_tlast=tlast,
          x_tlock=tlock, x_tcache=tcache, x_tprot=tprot, x_tqos=tqos, x_tregion=tregion,
          x_cnull=lambda t: Con('{}Null'.format(variant), t, value=Lit({'awsize': word_idx_bits, 'arsize': word_idx_bits})))


def AxiStreamType(name, data_bytes, id_bits=None, user_bits=None):
  tdata = UnsignedType('{}Data'.format(name), width=data_bytes*8)
  tkeep = UnsignedType('{}Keep'.format(name), width=data_bytes)
  ...
  TypeC(name, x_is_axi_stream=True,
        x_definition='{{>types/axis.part}}',
        x_format_ms='{{>formats/axis_ms.part}}',
        x_format_sm='{{>formats/axis_sm.part}}',
        x_tlogic=tlogic, x_tdata=tdata, x_tkeep=tkeep,
        x_tid=tid, x_tuser=tuser,
